<template>
  <div class="biji">
   ...
   <!-- <div>时间段:{{starttime}}至{{endtime}}</div> -->
    ...
    <div class="wrap">
      <div class="span" v-for="(item,index) in calendarList" :key="index" @click="click(item.count)" :class="gettime(item.count) ? 'noclick' 
      :getstartDate(item.count) ? 'noclick' 
      :item==''?'kong'
...
<script>
  export default {
    ...
    methods: {
      gettime(vel){
        let data = false
        this.getdata.dataList.map(item => { 
          if(item === vel){
            data = true
          }
      })
      ...
      getstartDate(item){
        let data = false
        if(item < this.getdata.startDate){
          data =true
        }
        return data
      },
