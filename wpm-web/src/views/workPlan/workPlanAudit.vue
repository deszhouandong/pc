  ...
  <div class="workplan">
    <div id="review" :style="viewShow == 'review'?'':'display:none'">
      ...
      <van-nav-bar  class="vanNavBar">
        ...
        <div slot="right" @click="workReviewClick" v-if="h3Active == 1 && !isOnBusiness"> <span>回顾</span> <van-icon name="arrow" /></div>
        <div slot="right" @click="workReviewClick" v-if="h3Active == 2 && !isOnBusiness" ><span>第{{nextNumberOfWeeks}}周</span><van-icon name="arrow" /></div>
      </van-nav-bar>

      <div :class="move" id='tabDiv' @touchstart="isOnBusiness?null:start($event)" @touchmove="isOnBusiness?null:moveFn($event)" @touchend="isOnBusiness?null:end($event)" >
        <bTripWorkPlan v-if="isOnBusiness"  :numberOfWeeks="numberOfWeeks" :numberOfWeeksNow="numberOfWeeksNow" :h3Active="h3Active" :onBusinessItem="onBusinessItem?onBusinessItem:{}"  
          :clickInput="clickInput" :wstatus="wstatus" :dstatus="dstatus" :did="did" :thisName="thisName" :year="year" :wid="wid" :adAcount="adAcount" :startDate="startDate" :wpsFrom="wpsFrom"
          :code="code"
        />
        <div v-if="!isOnBusiness">
          <div class="first">
            <div class="title">
              ...
              <!-- 显示访客次数 -->
              <div v-if="h3Active == 2 && isVisit" style="height:100%;width:1px;display:inline-flex;align-items:center;vertical-align: middle;">
                <div style="height:18px;width:0px;border-right:1px solid #007AFF;"></div>
              </div>

              <div v-if="h3Active == 2 && isVisit && visitNumber!=0" id="visitNumber" @click="toVisitorlist">
                  <van-image :src="imgSrc" />
                  <span class="visitNumber_span">{{visitNumber}}次拜访</span>
              </div>

            </div>
            <van-field v-model="wworkPlanReviewDes"
              :readonly="true"
              type="textarea"
              maxlength="200" :rows='firstRows'
          ...
          <div class="list" :style="!(h3Active == 1 && wstatus == 2 && isSuper == 1 && cookieName != adAcount&&!isOnBusiness)?'padding-bottom:0px;':''"
            @scroll="listScroll" ref="list" :dstatus="dstatus"
          >
            <ul class="main">
              <li  v-for="(item,index) in weekArr" :key="index">
                ...
                <div class="main_body">
                  <div class="itemUp itemText">
                    ...
                    />
                    <!-- 休假 -->
                    <div v-show="!item.disabledUp"  style="width:340px;height:18px;background:rgba(255,232,182,1);text-align:center;font-size:12px;font-weight:600;color:rgba(255,167,0,1);line-height:17px;"  >休假</div>
                  </div>
                  ...
                  <div class="itemDown itemText">
                    ...
                    />
                    <!-- 休假 -->
                    <div v-show="!item.disabledDown"  style="width:340px;height:18px;background:rgba(255,232,182,1);text-align:center;font-size:12px;font-weight:600;color:rgba(255,167,0,1);line-height:17px;"  >休假</div>
                  </div>
                  ...
                  <div class="circleBut" @click="()=>{isOnBusiness = true;onBusinessItem = item;}" v-if="(h3Active == 2)&& item.did == did &&isSuper == 1 && dstatus == 4 && cookieName != adAcount"  >
            ...
            <div id="opinion" v-if="(wstatus == 3 && h3Active == 1)">
              ...
              <div v-if="wverifiedUserName2">
                ...
                <div style="width:50%">
                  <p @click="clickInput(wplanDreviewVerifiedOpinio(2),'管理者意见',wverifiedUserName2,wdDate(2))">{{wplanDreviewVerifiedOpinio(2)}}</p>
                  <div><span>{{wverifiedUserName2}}</span><br/><span>{{wdDate(2)}}</span></div>
                </div>
      ...
      <div class="but_b" v-if="h3Active == 1 && wstatus == 2 && isSuper == 1 && cookieName != adAcount&&!isOnBusiness">
        <van-button plain type="primary"

         @click="audit"
          :disabled="wstatus == 3"
          :style="wstatus == 3?'background:#ccc;':''" class="withdraw"
        >审阅</van-button>
      </div>
        ...
        <!-- <div id="myPopup"> -->
          <ul class="main" style="padding:15px;">
            <li >
              ...
              <div class="itemUp itemText" style="font-size:16px;font-weight:400;color:rgba(72,72,72,1);line-height:18px; word-wrap: break-word;word-break: break-all;overflow: hidden;">
                {{popupText}}
              </div>
...
export default {
  ...
  data() {
    return {
      ...
      isOnBusiness:false,
      toBeRead:require("../../../static/imgs/toBeRead.png"), // 回顾待阅按钮图标
  ...
  mounted(){
    ...
    if(obj.chooseDate){ // 判断王甲戊是否串时间过来
      this.getWeekOfYear(new Date(parseInt(obj.chooseDate))) ;
    }
    this.isSuper = obj.isSuper?obj.isSuper:2;
    // this.selOneWeekPlan();
    if(Cookie.cookie.get('superAccount') == this.adAcount){ // 当查看上级时不显示访客记录入口
      this.isVisit = false;
    }
  ...
  methods: {
    ...
    placeholderReviewShow:function(item){ // 决定回顾的提示信息
      if( new Date(item.dateAll).getTime() > this.newDate.getTime() && this.h3Active == 2 && this.dstatus == 3){
        return '未到回顾时间';
      }else if( new Date(item.dateAll).getTime() < this.newDate.getTime() && this.h3Active == 2){
        return '未填写';
      }else if(this.h3Active == 2 && this.wstatus == 3 && new Date(item.dateAll).getTime() > this.newDate.getTime()){
        return '未到回顾时间';
    ...
    butSubmit(e){ // 提交和保存的事件委托
      ...
      if(this.h3Active == 1){
        ...
        let obj = {};
        this.weekArr.forEach(function(item,index){
        ...
        let _this = this;
        this.weekArr.forEach(function(item){
      ...
      this.$getData.selOneWeekPlan({
        ...
        if(res.code == 200 && res.data){
          ...
          data.workPlanDayList.forEach((item,index) => {
            ...
            if((!this.did && item.dworkPlanDate == this.newDate.format('yyyy-MM-dd')) || (this.did && this.did == item.did)){
              this.did = item.did;
  ...
  computed:{
    placeholderPlanShow:function(){ // 决定当日计划的提示信息
      if( (this.numberOfWeeks>=this.numberOfWeeksNow && this.numberOfWeeks<=(this.numberOfWeeksNow+3)) && this.h3Active == 1 && this.wstatus != 1 ){
        return '未填写';
      }
    },
    placeholderShow:function(){ // 决定周计划和回顾的提示信息
      if(this.h3Active == 2 && this.wstatus == 3 && this.numberOfWeeks > this.numberOfWeeksNow){
        return '未到回顾时间';
  ...
  watch:{
    ...
    isFirstRequest:function(){ // 监听是否第一次请求数据了
      this.listPosition();
    }
  }
};
...
#review{ // 计划与回顾的样式
  ...
  .but_b{ position: fixed;bottom: 0;width: 100%; box-shadow: 0 -1px 8px #3488c2;height:42px;z-index: 2;
    & button{ width:50%;float: left;height: 100%;font-size: 20px;line-height: 42px;font-weight: 600;border:0;
      &:nth-of-type(1){ background: #4B77AF;color: #fff; }
      &:nth-of-type(2){ background: #fff;color: #4B77AF; }
      &.withdraw{ width: 100%; }
    }
  }
  .first{
  ...
  .list{
    position: absolute;left: 0;right: 0;bottom: 0;top: 200px;
    // 新增样式 end
    width: 100%;overflow: scroll;background:#ececec;
    padding-bottom:64px;
...
#opinion{ // 管理者意见样式
  background: #fff;padding-top: 8px;padding-bottom:17px;
  min-height: 150px;
  // height: 180px; UI图大小大小
  ...
  // 12px
  &>div{ margin-top: 42px;padding: 0 22px 0 27px;min-height:120px;
    &>div{
      // width: 50%;
      position: relative;
      width: 100%;
...
#tabDiv{ // 轮播界面样式
  ...
  // 新增样式
  width:100%;position:fixed;top:70px;bottom:0;left:0;right:0;z-index: 1;padding-bottom: 46px;
